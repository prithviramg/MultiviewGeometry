...
def app():
    ...
    with st.sidebar:
        x = st.slider(label = "X Position", min_value = -3.0,max_value=  3.0, value= 0.0,step= 0.1, key="x_pos")
    with st.sidebar:
        y = st.slider(label = "Y Position", min_value = -3.0,max_value=  3.0, value= 0.0,step= 0.1, key="y_pos")
    with st.sidebar:
        z = st.slider(label = "Z Position", min_value = -3.0,max_value=  3.0, value= 0.0,step= 0.1, key="z_pos")
    with st.sidebar:
        pitch = st.slider(label = "pitch", min_value = -180,max_value=  180, value= -90,step= 1, key="pitch")
    with st.sidebar:
        roll = st.slider(label = "roll", min_value = -180,max_value=  180, value= 0,step= 1, key="roll")
    with st.sidebar:
        yaw = st.slider(label = "yaw", min_value = -180,max_value=  180, value= 0,step= 1, key="yaw")
    
    ######################## CAMERA ###############################  
    cam2world = pt.transform_from(R=pr.active_matrix_from_intrinsic_euler_xyz(e=[np.deg2rad(pitch),np.deg2rad(roll),np.deg2rad(yaw)]), 
                                  p = [x,y,z])
